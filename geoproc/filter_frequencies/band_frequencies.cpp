...
{
	if (S.so == geoproc::smoothing::smooth_operator::Laplacian) {
		bool res =
	...
	if (S.so == geoproc::smoothing::smooth_operator::BiLaplacian) {
		geoproc::smoothing::local_private::apply_twice_per_it
			(S.sw, S.lambda, -S.lambda, S.n_it, m, old_verts, new_verts);

		return old_verts;
	}
	if (S.so == geoproc::smoothing::smooth_operator::TaubinLM) {
...
namespace filter_frequencies {
	...
	(
		const std::vector<smoothing_configuration>& confs,
		const std::vector<float>& mus, size_t nt,
		TriangleMesh& m
	)
	{
		assert(mus.size() == confs.size() - 1);
